...
export default function PresetDCAInfo({ isConnected }: Props) {
  ...
  return (
    <Card>
      ...
      <CardHeader id="setting" title="프리셋" subtitle="하락장 세팅이 선택되었습니다" />
      <CardContent>
        ...
        (isConnected && !subscribeData?.productName.toLowerCase().includes("premium")) ? (
          ...
          <Stack className="w-full items-center gap-4">
            <FormTextField id="name" label="설정 보조지표" value="DCA 마틴" readOnly />
            <FormTextField id="profitRate" label="익절률" value="하락장 세팅" readOnly />
            <FormTextField id="lossRate" label="손절률" value="하락장 세팅" readOnly />
          </Stack>
        )}
      </CardContent>
    </Card>
